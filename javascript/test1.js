...
test("CreateNewTodo should add a new to-do item when Enter key is pressed and input is not empty", () => {
  ...
  inputField.value = "New To Do Item";

  const initialToDoLength = groups.tasks.length;

  const { itemElement } = handleAddTaskFieldEnter(mockEvent);
  ...
  equal(
    inputField.value,
    "",
    "the input field is cleared after adding a new item",
  );
...
test("CreateTodoElement Converts Text Property to Input Element Value", () => {
  ...
  equal(
    expected,
    resultText,
    "CreateTodoElement Converts Text Property to Input Element Value",
  );
});
...
test("updateItemAppearance adds the class 'hidden' to the item and hides completed item", () => {
  ...
  assert(
    itemElement.classList.contains("hidden"),
    "updateItemAppearance adds the class 'hidden' to the item and hides completed item",
  );
});
...
test("toggleCompleted toggles between show and hide", () => {
  ...
  assert(
    ...
    "toggleCompleted toggles between show and hide",
  );
});
...
test("remove an item from the list on click remove button", () => {
  ...
  const { itemElement, inputElement, removeButtonElement } =
  ...
  assert(
    !toDo.some((t) => t.id === item.id),
    "Item should be removed from the toDo array",
  );
});
...
test("update list order function to arrange completed items to the top of list", () => {
  ...
  const beforeOrder = Array.from(
    testListElement.querySelectorAll(".complete"),
